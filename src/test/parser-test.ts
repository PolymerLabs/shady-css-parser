...
describe('Parser', () => {
  ...
  describe('when parsing css', () => {
    it('can parse a basic ruleset', () => {
      expect(parser.parse(fixtures.basicRuleset)).to.be.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            'body',
    ...
    it('can parse at rules', () => {
      expect(parser.parse(fixtures.atRules)).to.containSubset(
        nodeFactory.stylesheet([
    ...
    it('can parse keyframes', () => {
      expect(parser.parse(fixtures.keyframes)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.atRule(
            'keyframes',
            'foo',
            nodeFactory.rulelist([
              ...
              nodeFactory.ruleset(
                ...
                nodeFactory.rulelist([
                  nodeFactory.declaration(
                    'fiz',
                    nodeFactory.expression('100px'),
                  ),
                  nodeFactory.declaration(
                    'buz',
                    nodeFactory.expression('true'),
                  ),
                ]),
              ),
            ]),
    ...
    it('can parse custom properties', () => {
      expect(parser.parse(fixtures.customProperties)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            ':root',
            nodeFactory.rulelist([
    ...
    it('can parse declarations with no value', () => {
      expect(parser.parse(fixtures.declarationsWithNoValue)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.declaration('foo', undefined),
          nodeFactory.declaration('bar 20px', undefined),
          nodeFactory.ruleset(
            'div',
            nodeFactory.rulelist([nodeFactory.declaration('baz', undefined)]),
          ),
        ]),
      );
    });
    ...
    it('can parse minified rulelists', () => {
      expect(parser.parse(fixtures.minifiedRuleset)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            '.foo',
            nodeFactory.rulelist([
    ...
    it('can parse psuedo rulesets', () => {
      expect(parser.parse(fixtures.psuedoRuleset)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            '.foo:bar:not(#rif)',
            nodeFactory.rulelist([
    ...
    it('can parse rulelists with data URIs', () => {
      expect(parser.parse(fixtures.dataUriRuleset)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            '.foo',
            nodeFactory.rulelist([
              nodeFactory.declaration(
                'bar',
                nodeFactory.expression('url(qux;gib)'),
              ),
            ]),
          ),
        ]),
    ...
    it('can parse pathological comments', () => {
      expect(parser.parse(fixtures.pathologicalComments)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            '.foo',
            nodeFactory.rulelist([
              nodeFactory.declaration(
                'bar',
                nodeFactory.expression('/*baz*/vim'),
              ),
            ]),
          ),
    ...
    it('disregards extra semi-colons', () => {
      expect(parser.parse(fixtures.extraSemicolons)).to.containSubset(
        nodeFactory.stylesheet([
          nodeFactory.ruleset(
            ':host',
            nodeFactory.rulelist([
  ...
  describe('when extracting ranges', () => {
    ...
    it('extracts the correct ranges for rulesets', () => {
      const expectRulesetRanges = (
        ...
        expect(rangeStrings).to.be.deep.equal(expectedRangeStrings);
        expect(selectorRangeStrings).to.be.deep.equal(
          expectedSelectorRangeStrings,
        );
      };
    ...
    it('extracts the correct ranges for rulelists', () => {
      ...
      expectRulelistRanges(fixtures.minifiedRuleset, [
        '{bar:baz}',
        '{vim:fet;}',
      ]);
      ...
      const expectAtRuleRanges = (
        ...
        const rangeParametersStrings = nodes.map((n) => {
          ...
          return cssText.substring(
            n.parametersRange.start,
            n.parametersRange.end,
          );
        });
        ...
        expect(rangeParametersStrings).to.be.deep.equal(
          expectedParameterRangeStrings,
        );
      };
...
function* getNodesOfType<K extends keyof NodeTypeMap>(
  node: Node,
  type: K,
